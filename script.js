...
function renderMathematicalVisualization(visualizationText, containerElement) {
  ...
  if (containerElement.dataset.initialized === 'true') {
    ...
    const existingContent = containerElement.querySelector('.math-content-container');
    if (existingContent) {
  ...
  lines.forEach(line => {
    ...
    if (line.startsWith('Step ') || line.toLowerCase().includes('setting key') || 
        line.toLowerCase().includes('initial state') || line.toLowerCase().includes('counter values')) {
      stepCounter++;
    ...
    // Check if this is a value entry
      ...
      if (line.startsWith('x[')) {
        ...
        groupType = 'carry';
      } else if (line.toLowerCase().includes('output') || line.toLowerCase().includes('keystream')) {
        groupType = 'output';
      } else {
        groupType = 'other';
      }
  ...
  mainContainer.style.fontFamily = '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
  ...
  steps.forEach((step, stepIndex) => {
    ...
    stepTitle.textContent = step.title.includes('Step') ? step.title : `Step ${step.id}: ${step.title}`;
    ...
    Object.keys(groupedByType).forEach(type => {
      ...
      if (type !== 'info' && type !== 'state') {
        ...
        sectionTitle.style.color = colorPalette[type]?.color || colorPalette.other.color;
      ...
      if (['x', 'c', 'temp'].includes(type)) {
        ...
        allValues.forEach(value => {
          ...
          valueElement.style.fontFamily = 'Consolas, Monaco, "Courier New", monospace';
          valueElement.style.fontSize = '11px';
          
          // Format hex values with a more subtle approach
          if (value.value.includes('0x')) {
            valueElement.innerHTML = value.value.replace(/0x([0-9A-F]+)/gi, 
      ...
      // Render carry values with more consistent styling
        ...
        groups.forEach(group => {
          group.values.forEach(value => {
            ...
            carryValueElement.style.fontFamily = 'Consolas, Monaco, "Courier New", monospace';
            carryValueElement.style.fontSize = '11px';
            
            // Format hex values with consistent styling
            if (value.value.includes('0x')) {
              carryValueElement.innerHTML = value.value.replace(/0x([0-9A-F]+)/gi, 
                '<span style="color:#4361ee;font-weight:500">0x</span><span style="font-weight:500">$1</span>');
            } else {
      ...
      // Render output/keystream values with more consistent styling
        ...
        groups.forEach(group => {
          group.values.forEach(value => {
            ...
            outputValue.style.fontFamily = 'Consolas, Monaco, "Courier New", monospace';
            ...
            // Format hex values with consistent styling
            if (value.value.includes('0x')) {
              outputValue.innerHTML = value.value.replace(/0x([0-9A-F]+)/gi, 
                '<span style="color:#4361ee;font-weight:500">0x</span><span style="font-weight:500">$1</span>');
            } else {
      ...
      // Render state information (Before/After) with more consistent styling
        ...
        groups.forEach(group => {
          ...
          stateLabel.style.color = group.stateType.startsWith('Before') ? colorPalette.x.color : colorPalette.c.color;
          ...
          if (group.stateData) {
            ...
            stateDataElement.style.fontFamily = 'Consolas, Monaco, "Courier New", monospace';
      ...
      // Render other info with more consistent styling
        ...
        groups.forEach(group => {
          ...
          if (group.value.toLowerCase().includes('note') || group.value.toLowerCase().includes('important')) {
  ...
  if (containerElement.dataset.initialized === 'true') {
    setTimeout(() => {
      const contentContainer = containerElement.querySelector('.math-content-container');
      if (contentContainer) {
    ...
    steps.forEach(step => {
      ...
      step.dataGroups.forEach(group => {
        if (group.type === 'state') {
          ...
          group.values.forEach(value => {
            downloadText += `${value.name} = ${value.value}${value.notes ? ', ' + value.notes : ''}\n`;
          });
        }
      });
...
function updateInternalStateVisualization() {
  ...
  const xStateValues = Array(8).fill(0).map(() => Math.floor(Math.random() * 0xFFFFFFFF).toString(16).padStart(8, '0'));
  const cStateValues = Array(8).fill(0).map(() => Math.floor(Math.random() * 0xF).toString(16));
  const counterValue = Math.floor(Math.random() * 0xFFFFFFFFFFFFFFFF).toString(16).padStart(16, '0');
  
  // Sample keystream output (replace with actual values)
  const keyStreamSample = Array(8).fill(0).map(() => Math.floor(Math.random() * 0xFFFF).toString(16).padStart(4, '0')).join(' ');
  ...
  let html = `
    <div class="rabbit-state-container">
      ...
      <div class="rabbit-panels">
        ...
        <div class="rabbit-panel">
          ...
          <div class="rabbit-x-grid">
            ${xStateValues.map((x, i) => `
              <div class="rabbit-x-item">
                <div class="rabbit-x-label">X[${i}]</div>
                <div class="rabbit-x-value">${x}</div>
                <div class="rabbit-x-meter"><div style="width: ${Math.floor(parseInt(x, 16) % 100)}%"></div></div>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      <div class="rabbit-steps">
        <div class="rabbit-step ${getCurrentStep() === 1 ? 'active' : ''}">1. Key Setup</div>
        <div class="rabbit-step ${getCurrentStep() === 2 ? 'active' : ''}">2. IV Setup</div>
        <div class="rabbit-step ${getCurrentStep() === 3 ? 'active' : ''}">3. Next-State</div>
        <div class="rabbit-step ${getCurrentStep() === 4 ? 'active' : ''}">4. Extract</div>
        <div class="rabbit-step ${getCurrentStep() === 5 ? 'active' : ''}">5. XOR</div>
      </div>
    </div>
...
function updateInternalStateVisualization() {
  ...
  <!-- Step 1: Key Setup -->
  <circle cx="50" cy="45" r="3" fill="${currentStep === 1 ? '#4361ee' : '#adb5bd'}"/>
  <line x1="50" y1="45" x2="70" y2="45" stroke="#adb5bd" stroke-width="1"/>
  <text x="73" y="48" font-size="6" text-anchor="start" font-weight="${currentStep === 1 ? 'bold' : 'normal'}" fill="#495057">1. Key Setup</text>
  <text x="73" y="56" font-size="5" text-anchor="start" fill="#6c757d">x[i] = k[i mod 8], c[i] = 0</text>
  
  <!-- Step 2: IV Setup -->
  <circle cx="50" cy="70" r="3" fill="${currentStep === 2 ? '#4361ee' : '#adb5bd'}"/>
  <line x1="50" y1="70" x2="70" y2="70" stroke="#adb5bd" stroke-width="1"/>
  <text x="73" y="73" font-size="6" text-anchor="start" font-weight="${currentStep === 2 ? 'bold' : 'normal'}" fill="#495057">2. IV Setup</text>
  <text x="73" y="81" font-size="5" text-anchor="start" fill="#6c757d">counter += IV[0..7] (mixed)</text>
  
  <!-- Step 3: State Iteration -->
  <circle cx="50" cy="95" r="3" fill="${currentStep === 3 ? '#4361ee' : '#adb5bd'}"/>
  <line x1="50" y1="95" x2="70" y2="95" stroke="#adb5bd" stroke-width="1"/>
  <text x="73" y="98" font-size="6" text-anchor="start" font-weight="${currentStep === 3 ? 'bold' : 'normal'}" fill="#495057">3. State Iteration</text>
  ...
  <circle cx="50" cy="120" r="3" fill="${currentStep === 4 ? '#4361ee' : '#adb5bd'}"/>
  <line x1="50" y1="120" x2="70" y2="120" stroke="#adb5bd" stroke-width="1"/>
  <text x="73" y="123" font-size="6" text-anchor="start" font-weight="${currentStep === 4 ? 'bold' : 'normal'}" fill="#495057">4. Extract Function</text>
  <text x="73" y="131" font-size="5" text-anchor="start" fill="#6c757d">s[i] = g(x[i] ⊕ x[i+5] ⊕ x[i+7] ≪ 16)</text>
  
  <!-- Step 5: Keystream Generation -->
  <circle cx="50" cy="145" r="3" fill="${currentStep === 5 ? '#4361ee' : '#adb5bd'}"/>
  <line x1="50" y1="145" x2="70" y2="145" stroke="#adb5bd" stroke-width="1"/>
  <text x="73" y="148" font-size="6" text-anchor="start" font-weight="${currentStep === 5 ? 'bold' : 'normal'}" fill="#495057">5. Keystream Generation</text>
  ...
  <g transform="translate(165, 37)">
    ...
    <g transform="translate(2, 2)">
      <rect x="0" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="7" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="14" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="21" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="28" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="35" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="42" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="49" y="0" width="6" height="6" fill="${currentStep >= 1 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <text x="28" y="4" font-size="4" text-anchor="middle" fill="#495057">X[0-7]</text>
    </g>
    <!-- Counter + Carry bits -->
    <g transform="translate(2, 10)">
      <rect x="0" y="0" width="38" height="6" fill="${currentStep >= 2 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <text x="19" y="4" font-size="4" text-anchor="middle" fill="#495057">Counter</text>
      <!-- Carry bits -->
      <rect x="39" y="0" width="2" height="6" fill="${currentStep >= 3 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="42" y="0" width="2" height="6" fill="${currentStep >= 3 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="45" y="0" width="2" height="6" fill="${currentStep >= 3 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="48" y="0" width="2" height="6" fill="${currentStep >= 3 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="51" y="0" width="2" height="6" fill="${currentStep >= 3 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
      <rect x="54" y="0" width="2" height="6" fill="${currentStep >= 3 ? '#d0ebff' : '#e9ecef'}" stroke="#adb5bd" stroke-width="0.5"/>
  ...
  <path d="M160 42 C130 42, 120 42, 105 42" stroke="#adb5bd" stroke-width="0.5" stroke-dasharray="1,1" fill="none"/>
        ...
        <div class="rabbit-right-column">
          ...
          <div class="rabbit-section rabbit-step-detail">
            <div class="rabbit-section-label">Current Step: ${stepDetails[currentStep-1].title}</div>
            <div class="rabbit-step-description">${stepDetails[currentStep-1].description}</div>
            <div class="rabbit-formula">${stepDetails[currentStep-1].formula}</div>
          </div>
          ...
          <div class="rabbit-steps">
            ${stepDetails.map((step, i) => `
              <div class="rabbit-step ${currentStep === i+1 ? 'active' : ''}" title="${step.title}">
                ${i+1}
              </div>
            `).join('')}
          </div>
  ...
  // Add event listener to the download button
  shadowRoot.querySelector('#downloadStateBtn').addEventListener('click', function() {
...
function updateStatistics() {
  ...
  document.getElementById("successPackets").textContent = successPackets/2;
...
function decryptData(hexData) {
  ...
  return fetch("rabbit_cipher.php", {
    ...
    .then((data) => {
      document.getElementById("connectionStatus").innerHTML = "Fetching Sensor Data...";
      document.getElementById("encryptionBadge").innerHTML = "Encrypted";
...
document.querySelectorAll("#visualization-tab .tab-content").forEach((c) => c.classList.remove("active"));
...
function initDashboard() {
  ...
  document.getElementById("encryptionStatus").className = "status-indicator status-inactive";
